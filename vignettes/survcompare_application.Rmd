...
#cross-validate CoxPH
cv1 <- survcox_cv(mydata2, mypredictors2, randomseed = 100)
# cross-validate Survival Random Forests ensemble
cv2 <- survsrfens_cv(mydata2, mypredictors2, randomseed = 100)
# compare the models 
...
print(round(cv1$test_pooled,4))

# Mean test performance of the Cox-PH and SRF ensemble:
print(round(cv2$test_pooled,4))

# Main stats 
round(compare_nonl$main_stats_pooled,4)
...
# run survcompare 
cv1<- survcox_cv(mygbsg, myfactors, randomseed = 42, repeat_cv = 5)
cv2<- survsrfens_cv(mygbsg, myfactors, randomseed = 42, repeat_cv = 5)
...
```
Internally validated test performance of CoxPH and SRF_ensemble over 5 repeated 3 fold cross-validations (inner k = 3 ). Mean performance:
               T C_score  AUCROC Calib_slope   sec
CoxPH        4.2  0.6751  0.7203      0.8614  1.47
SRF_ensemble 4.2  0.6890  0.7177      1.5925 52.88
Diff         0.0  0.0140 -0.0025      0.7311 51.41
pvalue       NaN  0.0014  0.6610      0.0000   NaN

Median performance:
               T C_score  AUCROC Calib_slope   sec
CoxPH        4.2  0.6701  0.7198      0.7889  1.47
SRF_ensemble 4.2  0.6900  0.7149      1.6513 52.88
Diff         0.0  0.0198 -0.0049      0.8625 51.41
pvalue       NaN  0.0014  0.6610      0.0000   NaN

SRF_ensemble has outperformed CoxPHby 0.014 in C-index.
...
Mean C-score: 
  CoxPH  0.6751(95CI=0.6608-0.6837;SD=0.0095)
  SRF_ensemble 0.689(95CI=0.6812-0.6971;SD=0.0064)
Mean AUCROC:
  CoxPH  0.7203(95CI=0.7027-0.7333;SD=0.0128)
SRF_ensemble 0.7177(95CI=0.711-0.7235;SD=0.0052)
 See other items as x$item. Items available:
results_mean, results_median, results_mean_train, return_models, bestparams, test, train, difftest, main_stats, main_stats_pooled, randomseed, useCoxLasso, model_name_base, model_name, cv
