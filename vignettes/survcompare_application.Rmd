...
The package checks whether there are considerable non-linear and interaction terms in the data, and quantifies their contributions to the models' performance. Using repeated nested cross-validation (that is, randomly but smartly slicing the data into the training and testing parts), the package:
  ...
  * Validates Survival Random Forest ensembled with the baseline Cox model. Here, `randomForestSRC::rfsrc()` is used.

  * Performs statistical testing of whether the Survival Random Forest ensemble outperforms the Cox model.
  
During this procedure, an expected predictive performance is evaluated by computing:
...
_Steyerberg, E. W., & Vergouwe, Y. (2014). Towards better clinical prediction models: seven steps for development and an ABCD for validation. European heart journal, 35(29), 1925-1931._
...
First, try to run sufficient number of repetitions (repeat_cv), at least 5, ideally 20-50 depending on the data heterogeneity and size. There are two possible outcomes: 1) "Survival Random Forest ensemble has outperformed CoxPH by ... in C-index", or 2) "Survival Random Forest ensemble has NOT outperformed CoxPH". 

  * If there is _no outperformance_, the test can justify the employment of CoxPH model and indicate a negligible advantage of using a more flexible model such as Survival Random Forest.
  
  * In the case of _outperformance_, a researcher can:
  
  1. Employ a more complex or flexible model.
  
  2. Look for the interaction and non-linear terms that could be added to the CoxPH and re-run the test. 
  
  3. Consider using the CoxPH model despite its underperformance if the difference is not large enough to sacrifice model's interpretability, or negligible in the context of a performed task.
...
Here, we got the results that _"Survival Random Forest ensemble has outperformed CoxPH by 0.0134 in C-index.
The difference is statistically significant with the p-value 0.00074325***. The supplied data may contain non-linear or cross-term dependencies, 
better captured by the Survival Random Forest."_

C-score: 
  CoxPH      0.6762(95CI=0.6327-0.7151;SD=0.0272)
  SRF_Ensemble 0.6895(95CI=0.648-0.7293;SD=0.0254)
  
This is the example that the outperformance is statistically significant and stable over different random splits, but not large in absolute terms. Therefore, the ultimate choice of a predictive model may be either, depending on how large the 0.01 in C-index is perceived by the model stakeholders.
