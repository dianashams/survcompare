...
#Let's take GBSG data from survival package. 
# We need to define "time" and "event" columns, and check missing values 
mygbsg <- gbsg
...
coxcv <- 
  survcox_cv(
    mygbsg, myfactors, predict_time, cv_number = 5, repeat_cv = 10)
...
# fit SRF
srfmodel1 <- 
  survsrf_train(traindf, myfactors, predict_time, inner_cv = 3,randomseed = 42)
...
# Further validation: repeated cross-validation. We keep low repeat_cv number for quick compilation, but even 3 repetitions of the 3-fold CVs show large variance of the SRF predictions, similar to that of CoxPH.
srfcv <- 
  survsrf_cv(
    mygbsg, myfactors, predict_time, cv_number = 3, repeat_cv = 3)
...
The final bit of the tutorial shows how SRF can be custom-tuned using survsrf_train() function with a supplied list of mtry, nodesize, and nodedepth values. There are more parameters in the `randomForestSRC::rfsrc()` function, however, these are the ones that are most often make difference in prediction accuracy. See also <https://cran.r-project.org/web/packages/randomForestSRC/randomForestSRC.pdf>.
