...
stack_deepsurv_train <-
  function(df_train,
    ...
    for (cv_iteration in 1:k_for_oob) {
      ...
      deepsurv.cv <-
        deepsurv_train(df_train = data_train,
                      predict.factors = predict.factors,
                      fixed_time = fixed_time,
      ...
      # predict for data_oob
      ml_predict_oob <-
        deepsurv_predict(trained_model = deepsurv.cv,
                        newdata = data_oob,
                        predict.factors = predict.factors,
                        fixed_time= fixed_time)
...
stack_deepsurv_predict <-
  function(trained_object,
    ...
    predictdata$ml_predict <-
      deepsurv_predict(trained_model = trained_object$model_base_ml,
...
stack_deepsurv_cv <- function(df,
  ...
  output <- surv_CV(
    ...
    train_function = stack_deepsurv_train,
    predict_function = stack_deepsurv_predict,
