...
#' #' @param oob TRUE/FALSE use out-of-bag prediction - discontinued
#' #' @examples
...
#'            fixed_time,
#'            oob = FALSE) {
#'     # computes event probabilities from the SRF model (1- survival prob)
#'
#'     # if (oob) {
#'     #   # this seems to do something that is not just OOB predictions from RF,
#'     #   # but rather new KM curves based on test set (and it uses ys from the test ... )
...
#'     #   #based solely on the training data, but where predicted values are obtained from test data. Error rates
#'     #   #and VIMP are calculated by bootstrapping the test data and using out-of-bagging to ensure unbiased
#'     #   #estimates.
#'     #   predicted_matrix <- predict(rfmodel,newdata = df_to_predict,oob = TRUE,outcome = "test")
#'
#'     print("srf_survival_prob_for_time") #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#'
#'     predictions <- predict(rfmodel, newdata = df_to_predict)
#'     train_times <- predictions$time.interest
#'     predicted_matrix <- predictions$survival
#'
#'     # no extrapolation => return NaN
#'     if (fixed_time > max(train_times)) {
#'       print("41") #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#'       print(dim(df_to_predict)[1])
#'       predict_eventprob <- rep(NaN, dim(df_to_predict)[1])
#'       print(predict_eventprob)
#'       return(predict_eventprob)
#'     }
#'
#'     # constant interpolation for in-between times (as in K-M curves)
#'     f <- function(i) {
#'       approxfun(train_times, predicted_matrix[i, ], method = "constant")(fixed_time)
#'     }
#'     predict_eventprob <- 1 - unlist(lapply(1:dim(predicted_matrix)[1], f))
#'
#'     #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#'     # Sometimes, SRF omits observations in newdata for which survival is not computed
#'     # this is dangerous as it is not clear which ones were missed.
#'     # as a work-around, we go one by one and add na.action "na.random" to be safe for spurious errors
#'     if(length(predict_eventprob)!= dim(df_to_predict)[1]) {
#'       predict_eventprob <- rep(NaN, dim(df_to_predict)[1])
#'       cat("!")
#'       for (i in 1:dim(df_to_predict)[1]){
#'         predictions <-
#'           predict(rfmodel, newdata = df_to_predict[i, ], na.action = "na.random")
#'         predict_eventprob[i] =
#'           approxfun(train_times,
#'                     predictions$survival[1, ],
#'                     method = "constant")(fixed_time)
#'       }
#'     }
#'     return(predict_eventprob)
#'   }
...
#'   #   if (verbose) {
#'   #     print("Warning - some nodesize parameters are > 1/5 of
#'   #           the sample size and may prevent effective tree building.")
#'   #   }
#'   # }
#'
...
#'
#'   if (dim(grid_of_values)[1] == 0) {return(list())}
#'
...
#'         validation_stats <-
#'           surv_validate(y_predict = y_predicted, predict_time = fixed_time,
#'                         df_train = df_train_cv,df_test = df_test_cv,
#'                         weighted = TRUE)
#'
...
#'         seed = randomseed
#'       )
#'       # compute performance metrics for all based on OOB predictions
#'       y_predicted <-
#'         1 - srf_survival_prob_for_time(
#'           rfmodel = rf.dt,
#'           df_to_predict = df_tune,
#'           fixed_time = fixed_time,
#'           oob = TRUE
#'         )
#'       #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#'       if (sum(is.na(y_predicted) >0)){print("y_predicted")}
#'
#'       validation_stats <-
#'         surv_validate(
#'           y_predict = y_predicted,
#'           predict_time = fixed_time,
#'           df_train = df_tune,
#'           df_test = df_tune,
#'           weighted = TRUE
#'         )
#'       if (sum(is.na(y_predicted) >0)){print("y_predicted1")}
#'       modelstats[[i]] <- c(
#'         "mtry" = mtry_i,
#'         "nodesize" = nodesize_i,
#'         "nodedepth" = nodedepth_i,
...
#'   mtry <- mtry[mtry <= length(predict.factors)]
#'   nodesize <- seq(min(15, round(n/6 - 1, 0)), max(min(n/10, 50), 30), 7)
#'   nodedepth <- c(5, 25)
#'
#'   # update them if given in tuningparams
#'   if (length(tuningparams)>0) {
#'     if (!is.null(tuningparams$nodesize)) {nodesize = tuningparams$nodesize}
#'     if (!is.null(tuningparams$mtry)) {mtry = c(tuningparams$mtry)}
#'     if (!is.null(tuningparams$nodedepth)){nodedepth = tuningparams$nodedepth}
#'   }
#'
#'   if (verbose) {
#'     print("Tuning the following mtry, nodesize, and nodedepth:",
#'           paste(mtry, nodesize, nodedepth))
#'   }
#'
...
#'       verbose = TRUE #!!!!!!!!!!!!!!!!!!!!!!!
#'     )
#'     nodesize_best <- as.integer(tune1$bestcindex["nodesize"])
#'     nodedepth_best <- as.integer(tune1$bestcindex["nodedepth"])
#'
...
#'       oob = oob
#'     )
#'     mtry_best <- tune2$bestcindex["mtry"]
#'     best_combo_stat <- tune2$bestcindex
...
#'       oob = oob
#'     )
#'     nodesize_best <- tuneall$bestcindex["nodesize"]
#'     nodedepth_best <- tuneall$bestcindex["nodedepth"]
#'     mtry_best <- tuneall$bestcindex["mtry"]
#'     best_combo_stat <- tuneall$bestcindex
#'     modelstatsall <- tuneall$modelstats
#'   }
#'
#'   final.rfs <- randomForestSRC::rfsrc(
#'     as.formula(paste("Surv(time, event) ~",
#'                      paste(predict.factors, collapse = "+"))),
#'     data = df_train,
#'     nodesize = nodesize_best,
#'     # this is AVERAGE size, so we want this to be quite high
...
#'     model_args = list("tuningparams" = tuningparams, "oob" = oob, "fixed_time"= fixed_time),
