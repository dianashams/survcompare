...
summary.survcompare <-
  function(output_object, useCoxLasso) {
    ...
    if (x$difftest["pvalue", "C_score"] < 0.05) {
      ...
      t3<- ". The supplied data may contain non-linear or cross-term dependencies, better captured by the Survival Random Forest.\n"
    }else{
      ...
      t3<- ". The data may NOT contain considerable non-linear or cross-term dependencies, better captured by the Survival Random Forest.\n"
    }
