...
surv_CV <-
  function(
    ...
    print(paste("Cross-validating Survival Random Forest - Cox model ensemble ( ",
    ...
    for (rep_cv in 1:repeat_cv) {
      ...
      for (cv_iteration in 1:cv_number) {
        ...
        modelstats_test[[cv_iteration + (rep_cv-1)*cv_number]] <-
          surv_validate(y_predict_test, fixed_time, df_train_cv,
                  df_test_cv, weighted = 1)
        modelstats_train[[cv_iteration + (rep_cv-1)*cv_number]] <-
          surv_validate(y_predict_train,fixed_time,df_train_cv,
    ...
    stats_summary <- function(x){
      ...
      as.data.frame(round(cbind(
        ...
        "95CILow" = apply(x, 2, quantile, 0.025, na.rm=1)),4))
    }
    ...
    output$call <- Call
    output$cv <- c(cv_number, inner_cv, repeat_cv)
